...
window.onload = function () {
  ...
  function init() {
    ...
    canvas.addEventListener("mousemove", sensorChange);
  ...
  function drawFrame() {
    ...
    context.font = "20px Verdana";
    context.fillText(
      "Rotate the POT to control the cannon and click to shoot!",
      10,
      37
    );
  }
  ...
  // On mouse movement
  function sensorChange(sensorValue) {
    ...
    */
    player.angle = 8 + sensorValue * 360 + 180;
  }

  //client socket
  const socket = io();
  socket.on("sensorChange", (value) => {
    sensorChange(value);
  });
